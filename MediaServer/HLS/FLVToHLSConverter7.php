...
class FLVToHLSConverter7
{
    ...
    private function processVideoFrame(VideoFrame $frame, $relativeTime)
    {
        ...
        if ($avcData['avcPacketType'] == Flv::AVC_PACKET_TYPE_SEQUENCE_HEADER) {
            $this->videoSequenceHeader = $avcData['data'];
            $this->spsPpsAnnexB = $this->extractSPSPPS($avcData['data']);
            return;
        }
        ...
        if ($isKeyFrame) {
            if ($relativeTime - $this->lastKeyframeTimestamp >= $this->segmentDuration * 1000) {
                $this->startNewSegment($relativeTime);
                $this->lastKeyframeTimestamp = $relativeTime;
            }
        }

        $nalus = $this->convertToAnnexB($avcData['data']);
        if ($isKeyFrame) {
            // 🚩 关键帧前，prepend SPS/PPS
            $nalus = $this->spsPpsAnnexB . $nalus;
        }

        if ($this->tsFileHandle) {
            $this->writeVideoToTS($nalus, $relativeTime, $isKeyFrame);
        }
    }

    private function extractSPSPPS($seqHeader)
    {
        $offset = 5;
        $spsPps = '';

        $numSPS = ord($seqHeader[$offset]) & 0x1F; $offset++;
        for ($i = 0; $i < $numSPS; $i++) {
            $spsLen = unpack('n', substr($seqHeader, $offset, 2))[1]; $offset += 2;
            $spsPps .= "\x00\x00\x00\x01" . substr($seqHeader, $offset, $spsLen);
            $offset += $spsLen;
        }

        $numPPS = ord($seqHeader[$offset]); $offset++;
        for ($i = 0; $i < $numPPS; $i++) {
            $ppsLen = unpack('n', substr($seqHeader, $offset, 2))[1]; $offset += 2;
            $spsPps .= "\x00\x00\x00\x01" . substr($seqHeader, $offset, $ppsLen);
            $offset += $ppsLen;
        }
    ...
    private function convertToAnnexB($nalu)
    {
        $result = '';
        $offset = 0;
        $len = strlen($nalu);
