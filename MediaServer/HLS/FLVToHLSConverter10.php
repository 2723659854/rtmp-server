 ...
 * @version 1.0.6 (patched)
 * @note 当前版本在原有基础上修复了 AVC sequence header(SPS/PPS) 解析错误导致的马赛克问题
...
class FLVToHLSConverter6
{
    ...
    private function processVideoFrame(VideoFrame $frame, $relativeTime)
    {
        ...
        if ($this->tsFileHandle) {
             ...
             * 视频帧需要转化为Annex‑B格式
             * 关键改动：
             *  - 对于序列头（AVCDecoderConfigurationRecord），不能直接用 toAnnexB()
             *  - 需要按规范解析出 SPS/PPS，再拼接起始码
             */
            if ($isKeyFrame) {
    ...
    /**
     * 将 AVCDecoderConfigurationRecord(FLV AVC sequence header) 解析为 Annex‑B SPS/PPS
     * @param string $seq
     * @return string Annex‑B 格式的 SPS/PPS 数据（带 0x00000001 起始码）
     */
    private function avcSequenceHeaderToAnnexB(string $seq): string
    {
        ...
        // 接下来 1 字节: 3 bits '111' + 5 bits numOfSPS
     ...
     * @note 这个方法生成的ts切片全部能够播放，但是全是马赛克（已修复为正常清晰画面）
     */
