...
- 🔌 **内置双 Web 推流测试页**：
    - `push.html`：屏幕共享（含窗口/标签页）实时推流
    - `flv_push.html`：本地 FLV/MP4 文件伪直播推流
...
### 4. 推流测试（无需安装任何推流软件）
浏览器打开以下任一页面即可推流：
- **实时推流**：`http://127.0.0.1/push.html` – 使用摄像头或屏幕共享（可共享整个屏幕、特定窗口或浏览器标签页）
- **伪直播推流**：`http://127.0.0.1/flv_push.html` – 选择本地 FLV/MP4 文件，推送模拟直播
...
项目内置两个 WebSocket 推流测试页面，无需安装任何推流软件，直接在浏览器中即可推流。

### 1. `push.html` – 屏幕共享实时推流

**访问地址**：`http://127.0.0.1/push.html`

**功能说明**：通过浏览器获取屏幕内容（支持整个屏幕、特定窗口或浏览器标签页），实时推送到服务器，适合现场直播、视频会议等场景。

**使用步骤**：
1. 启动源站服务（`php server.php`）
2. 浏览器打开 `http://127.0.0.1/push.html`
3. 推流地址默认 `ws://127.0.0.1:8501/live/stream`（可自定义应用名/频道名）
4. 点击「开始推流」按钮，授权后，页面将实时推送音视频数据

---

### 2. `flv_push.html` – 本地文件伪直播推流 ⭐

**访问地址**：`http://127.0.0.1/flv_push.html`

**功能说明**：选择本地的 FLV 或 MP4 视频文件，通过 WebSocket 推流，模拟直播频道。适用于：
- 测试直播系统稳定性
- 模拟电视台轮播
- 无需摄像头和麦克风的自动化直播
...
**使用步骤**：
1. 启动源站服务（`php server.php`）
2. 浏览器打开 `http://127.0.0.1/flv_push.html`
3. 推流地址默认 `ws://127.0.0.1:8501/live/stream`
4. 点击「选择文件」选取本地 FLV 或 MP4 文件
5. 点击「开始推流」，文件将循环推送，直至手动停止
...
> **注意**：Web 推流基于 WebSocket 协议，仅支持 H.264 视频编码和 AAC/MP3 音频编码。请确保您的浏览器或文件符合编码要求。
...
- 使用 ffplay：`ffplay ws://127.0.0.1:8501/live/stream.flv`
...
项目提供了两个开箱即用的浏览器推流页面：
- **实时推流**：`http://127.0.0.1/push.html` – 摄像头 / 屏幕共享（含窗口/标签页）
- **伪直播推流**：`http://127.0.0.1/flv_push.html` – 本地 FLV/MP4 文件循环推流

在页面中配置推流地址 `ws://127.0.0.1:8501/live/stream` 即可开始。
...
├── push.html                         # Web推流测试页（摄像头/屏幕共享，含窗口/标签页）
├── flv_push.html                     # Web推流测试页（本地FLV/MP4文件伪直播）
├── *.html                            # Web播放页面
└── README.md
```
...
## 工具包
已将本本项目的协议转换独立成为一个工具包`xiaosongshu/flv2mp4`,地址`https://github.com/2723659854/flv2mp4` ，支持flv,mp4,hls转码，flv和file网关，以及flv/mp4静态文件推流客户端。
## 📄 开源协议
