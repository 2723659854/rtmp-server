...
upstream filegateway_cluster {
    server <your-server-ip>:8100;
    server <your-server-ip>:8101;
    server <your-server-ip>:8102;
}
server {
    listen 80;
...
根据您的描述，我设计了一份**十万级并发集群部署架构图**及其详细说明。该架构突出高可用、弹性伸缩、多活容灾，并强调源/边缘节点角色可互换、自动同步、多备份录制等特性。您可以直接将以下内容作为新章节插入 README（建议放在“系统整体架构”之后）。
